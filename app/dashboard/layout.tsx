...
export default function DashboardLayout({
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      {/* Sidebar */}
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-slate-200 transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 transition-transform duration-300 ease-in-out flex flex-col`}>
        {/* Logo Section */}
        <div className="flex items-center gap-3 p-5 border-b border-slate-100">
          <Link href="/dashboard" className="flex items-center gap-3 group">
            <div className="w-10 h-10 bg-blue-50 rounded-xl flex items-center justify-center shadow-sm overflow-hidden">
              <Image
                src="/logo.png"
            ...
            <div>
              <h1 className="text-slate-800 font-semibold text-sm group-hover:text-blue-600 transition-colors">
                Advent Hope Academy
              </h1>
          ...
          <div className="space-y-1">
            {filteredMenuItems.map((item) => {
              ...
              return (
                ...
                >
                  {IconComponent && (
                    <IconComponent className={`w-5 h-5 flex-shrink-0 ${active ? 'text-blue-600' : 'text-slate-400 group-hover:text-slate-600'}`} />
                  )}
        ...
        <div className="p-4 border-t border-slate-100">
          ...
          <button
            ...
            className="flex items-center justify-center gap-2 w-full py-2.5 text-slate-500 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all duration-200 text-sm font-medium"
          >
